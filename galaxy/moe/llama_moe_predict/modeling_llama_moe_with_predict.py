...
from .configuration_llama_moe_predict import LlamaMoEPredictConfig

logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "LlamaMoEPredictConfig"
...
class BaseMoEModelOutputWithPast(ModelOutput):
    ...
    all_gate_inputs: Optional[Tuple[torch.FloatTensor]] = None
    all_gate_outputs: Optional[Tuple[torch.FloatTensor]] = None
...
class MoEMlpOutput(ModelOutput):
    ...
    gate_input: Optional[Tuple[torch.FloatTensor]] = None
    gate_output: Optional[Tuple[torch.FloatTensor]] = None
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaMoEPredictConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class TopKBalancedNoisyGate(nn.Module):
    ...
    def forward(self, x):
        ...
        # 如果是训练，需要平衡每个expert的负载量，loss添加惩罚项
        if self.use_balance:
...
class LinearGLUExperts(nn.Module):
    ...
    def __init__(
        ...
        self.num_experts = num_experts
        # print(self.in_features, self.hidden_features, self.out_features)

        # size_exports [1, num_experts]: 定义每一层的每一个expert的大小
        ...
        if bias:
            ...
            # 这确保了模型类的接口保持一致，这意味着模型的其他部分可以始终假设 
            # bias_gate、bias_up 和 bias_down 这些属性存在，即使它们可能没有实际的参数值。
...
class UniversalCalculator(nn.Module):
    ...
    def forward(
        ...
        # 生成从0到seq_len-1的序列，序列每个元素重复num_selects次：[0,0,1,1,2,2,..,]
        # [seq_len*num_selects]
...
class BaseMoELayer(nn.Module):
    def __init__(self):
        ...
        self.predict_gate: TopKBalancedNoisyGate
    ...
    def forward(self, x) -> MoEMlpOutput:
        ...
        # [bs*seq_len, hidden_size] 因为 bs=1，因此都为[seq_len, hidden_size]
        ...
        return MoEMlpOutput(
            ...
            gate_input=gate_outputs.get("gate_input" ),
            gate_output = gate_outputs.get("gate_output"),
        )
...
class LlamaMoEDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMoEPredictConfig, layer_index):
        super().__init__()
    ...
    def forward(
        ...
        outputs = (
            ...
            # 每一个decoder layer，每个expert处理的tokens数量
            mlp_outs.gate_load,
            # 每个expert的重要性程度，和gate_load呈现正相关
        ...
        if use_cache:
         ...
         # 增加gate的输入输出
        outputs +=     (mlp_outs.gate_input,)
        outputs +=      (mlp_outs.gate_output,)
...
class LlamaMoEPreTrainedModel(PreTrainedModel):
    config_class = LlamaMoEPredictConfig
    ...
    # 当模型的状态字典（state dict）被加载时，通常会将所有张量（tensors）移动到指定的设备上(CPU/GPU)
    # 这个字段设置那些键不被移动
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LlamaMoEModelPredict):
            module.gradient_checkpointing = value


class LlamaMoEModelPredict(LlamaMoEPreTrainedModel):
    def __init__(self, config: LlamaMoEPredictConfig):
    ...
    # 构造因果掩码
    # inputs: (bsz, target_length), output mask: (batch_size, 1, target_length, target_length)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # 推理执行
            # 当 use_cache 被设置为 True 时，模型在生成过程中会保存一些关键的中间结果（如 past_key_values）
            else:
            ...
            all_gate_inputs += (layer_outputs[-2],)
            all_gate_outputs += (layer_outputs[-1],)
        ...
        return BaseMoEModelOutputWithPast(
            ...
            all_gate_inputs = all_gate_inputs,
            all_gate_outputs=all_gate_outputs,
        )
...
class LlamaMoEForCausalLMPredict(LlamaMoEPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        print("init LlamaMoEForCausalLMPredict")
        self.model = LlamaMoEModelPredict(config)
        self.pretraining_tp = config.pretraining_tp
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        return MoECausalLMOutputWithPast(
            ...
            all_gate_inputs=outputs.all_gate_inputs,
