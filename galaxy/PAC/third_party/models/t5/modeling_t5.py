...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, adapter_config=None, lora_config=None):
        super().__init__()
        self.bitfit = adapter_config.bitfit if adapter_config is not None else False 
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None, lora_config=None):
        super().__init__()
        self.SelfAttention = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, adapter_config=adapter_config, lora_config=lora_config)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, adapter_config=None, lora_config=None):
        super().__init__()
        self.EncDecAttention = T5Attention(config, has_relative_attention_bias=False, adapter_config=adapter_config, lora_config=lora_config)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None, lora_config=None):
        ...
        self.layer.append(T5LayerSelfAttention(config,has_relative_attention_bias=has_relative_attention_bias, adapter_config=adapter_config, lora_config=lora_config))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, adapter_config=adapter_config, lora_config=lora_config))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None, lora_config=None):
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None, lora_config=None):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared, lora_config=lora_config)
        ...
        self.decoder = T5Stack(decoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared, lora_config=lora_config)
