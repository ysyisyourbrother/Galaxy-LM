...
class QSTOPTForSequenceClassification(OPTPreTrainedModel):
    def __init__(self, llm: OPTForSequenceClassification, config, QSTConfig):
        super().__init__(config)
        self.num_labels = llm.num_labels
        self.hidden_size = config.hidden_size
        self.word_embed_proj_dim = config.word_embed_proj_dim
        self.model = QSTOPTModel(llm.model, config, QSTConfig)
        self.lm_head_z = nn.Parameter(torch.zeros(self.hidden_size ))
        self.upsample = nn.Linear( int(self.hidden_size  / QSTConfig.r), self.word_embed_proj_dim  )
        self.score = nn.Linear(self.word_embed_proj_dim , self.num_labels, bias=False)
        del llm
        # Initialize weights and apply final processing
        # self.post_init()
